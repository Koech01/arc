// Arc Demo Workspace Seeder — single-file, idempotent script.
//
...
var seeder = new DemoWorkspaceSeeder(scope.ServiceProvider);
await seeder.RunAsync();
Console.WriteLine("✔ Demo workspace ready. Login: demo@arc.com / DemoArc2026!");

// ═══════════════════════════════════════════════════════════════════════════
// Seeder
// ═══════════════════════════════════════════════════════════════════════════

file sealed class DemoWorkspaceSeeder(IServiceProvider sp)
{
    // ── Fixed demo identity ───────────────────────────────────────────────
    const string Email = "demo@arc.com";
    ...
    readonly IUserRepository _users = sp.GetRequiredService<IUserRepository>();
    readonly IPasswordHashingService _hasher = sp.GetRequiredService<IPasswordHashingService>();
    readonly IUserPreferencesRepository _prefs = sp.GetRequiredService<IUserPreferencesRepository>();
    readonly ILLMConfigurationRepository _llms = sp.GetRequiredService<ILLMConfigurationRepository>();
    readonly IWorkflowRepository _workflows = sp.GetRequiredService<IWorkflowRepository>();
    readonly IExecutionTemplateStore _templates = sp.GetRequiredService<IExecutionTemplateStore>();
    readonly IExecutionResultStore _executions = sp.GetRequiredService<IExecutionResultStore>();
    readonly IAuditLogger _audit = sp.GetRequiredService<IAuditLogger>();
    readonly IGoldenExecutionStore _goldens = sp.GetRequiredService<IGoldenExecutionStore>();
    readonly IRegressionGateRepository _gates = sp.GetRequiredService<IRegressionGateRepository>();
    readonly IWebhookRepository _webhooks = sp.GetRequiredService<IWebhookRepository>();
    readonly INotificationRepository _notifications = sp.GetRequiredService<INotificationRepository>();
    readonly IDatabaseContext _db = sp.GetRequiredService<IDatabaseContext>();
    readonly ILogger _log = sp.GetRequiredService<ILogger<DemoWorkspaceSeeder>>();
    ...
    async Task CleanupAsync()
    {
        ...
        if (existing is null)
        {
            _log.LogInformation("No existing demo user — fresh seed");
            return;
        }

        _log.LogInformation("Removing existing demo user and all owned data…");
    ...
    async Task SeedUserAsync()
    {
        ...
        _log.LogInformation("Created demo user {Email}", Email);
    }
