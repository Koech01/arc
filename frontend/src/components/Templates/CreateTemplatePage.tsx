...
export function CreateTemplatePage() {
  ...
  const [tasks, setTasks] = useState<TemplateTask[]>([
    { id: 'task-1', name: '', agentType: 'llm', prompt: '', config: {}, dependencies: [] }
  ]);
  ...
  const addTask = () => {
    ...
    const newId = `task-${nextTaskNumber}`;
    setTasks([...tasks, { id: newId, name: '', agentType: 'llm', prompt: '', config: {}, dependencies: [] }]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await templateApi.create({ name, description, triggerType, llmConfigId: llmConfigId || undefined, tasks });
      toast.success('Template created successfully', { position: 'top-center' });
      navigate('/templates');
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Failed to create template', { position: 'top-center' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-6 w-full">
        <Card className="pb-4 max-w-4xl">
          <CardHeader className="pl-4">
            ...
            <CardDescription>Create a reusable workflow template with variable support</CardDescription>
          </CardHeader>
          <CardContent>
